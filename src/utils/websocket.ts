...
function validateLogEntry(log: any): boolean {
    ...
    if (log.level && !['debug', 'info', 'warn', 'error', 'fatal'].includes(log.level)) return false;
...
export function createWebSocketServer(httpServer: Server): void {
    ...
    wss.on('connection', async (ws, req) => {
        ...
        if (!auth) {
            wsLogger.warning('WebSocket authentication failed', { token: token ? 'provided' : 'missing' });
        ...
        ws.on('message', async (data) => {
            try {
                ...
                if (!Array.isArray(logs)) {
                    wsLogger.error('Logs must be an array', { receivedType: typeof logs });
                    ws.send(JSON.stringify({ error: 'Invalid log format - expected array' }));
                    return;
                }
